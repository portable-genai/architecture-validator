...
// console. What was actually shipped was dead markup: a `script-src` with no nonce blocks Next's
// inline hydration bootstrap, `__next_f` never fills, React never attaches, and no control on the
// page does anything.
//
...
//
//   0. Every directive the standard requires is present, and none of them is empty. An empty
//      directive is a CSP parse error, which browsers discard -- so a policy that looks stricter
//      in the source can be weaker in the browser. This console's pre-fix policy was
//      `frame-ancestors 'self'` and NOTHING else: no default-src, no object-src, no base-uri.
//   1. The response carries a nonce in `script-src`.
//   2. EVERY `<script>` tag in the document carries that same nonce.
//
// Rule 2 is the one that matters, and it is the one a header assertion cannot express. The header
...
// nonce, and because `'strict-dynamic'` disables the `'self'` fallback, that combination blocks
// strictly MORE than the unfixed policy did. Header and markup have to agree, and only the
// markup knows.
//
...
// A cold `next start` on CI has taken well over 60s, and a boot timeout reported as a CSP failure
// is the worst kind of flake: it points the reader at the wrong file.
const BOOT_TIMEOUT_MS = 90_000;
const POLL_MS = 250;

/** Directives whose ABSENCE is this console's original fleet defect, not a bad value. */
