 ...
 * `X-Search-Used: true`. Both headers are emitted, and exposed to this cross-origin reader, by
 * the service (`install_answer_provenance` in `api/app.py`).
 *
 ...
 * of its own would be silently refused, and the pills would render nothing. (The banner these
 * replace once fetched `/api/agent/healthz`, a same-origin route this console does not ship,
 * and rendered nothing on every page load until that was found.)
 */
