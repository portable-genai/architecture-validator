"""IdentityPort -- the authentication boundary, re-exported rather than redeclared.

The hexagon boundary for authentication. The API layer hands the adapter a
:class:`~architecture_validator.domain.identity.RequestContext` (the request headers) and gets back
a verified :class:`~architecture_validator.domain.identity.Principal`, or an ``IdentityError``. The
active profile picks the adapter:
...
The Protocol body is NOT written out here. It comes from :mod:`hex_service_kit.identity`,
for the same reason the value objects do: a Protocol copied into N repositories is N
Protocols, and only one of them gets fixed when a defect is found. Typing-only import, so
this costs the offline profile nothing.
