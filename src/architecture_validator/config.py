...
#: Profiles that mean "running on managed cloud infrastructure", for :attr:`Settings.runtime`
#: (the title of the console's model pill).
...
#: answers the pill's "where does this run" question: ``platform`` binds the same review
...
#: the empty string when it keeps none there. It must be the setting the managed adapter itself
#: reads (``request.model or self._models.reasoning`` in ``adapters/gcp/gemini_llm.py``).
#:
...
#: What the ``local`` profile's deterministic generator answers as. ``generator_model`` reports
#: it under ``local`` and the adapter notes the same string for ``X-Answered-By``, so the pill
#: reads one value before and after an answer, and neither ever names a managed model.
...
#: The switch for review routing, the one cheap runtime control this service has (it binds no
#: guardrail and no redaction port), read in three states: unset is ON (the reference posture
#: keeps cheap controls on), a boolean value wins, and an emptied or unrecognised value refuses
#: at boot. See the fleet's runtime-control contract.
REVIEW_ROUTING_ENV = "ARCH_VALIDATOR_REVIEW_ROUTING"
...
#: rather than a hand-off that fails, silently, on every validation report or residency scan.
...
class Settings:
    ...
    def generator_model(self) -> str:
        """WHICH model the bound generator would call, as the console's model pill first states it.
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    """Review routing on under a networked profile must name its console, checked at boot.

    Review routing on with no console named used to fail every hand-off silently behind the
    service's ``contextlib.suppress``. That is a configuration error, so it refuses here and
    says how to either name the console or switch routing off out loud.
    """
    if settings.profile not in _NETWORKED_PROFILES:
        return
    if settings.controls.review_routing and optional_setting(HUMAN_REVIEW_URL_ENV) is None:
        raise ConfiguredEmptyError(
