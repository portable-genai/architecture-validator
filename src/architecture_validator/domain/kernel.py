...
class LlmRequest:
    ...
    #: ``None`` sends no temperature at all: the model samples at its own default. Pinned
    #: (``0.0``) only where the output is extracted, classified, scored or compared; a call
    #: that drafts, narrates or explains leaves it free. Omitted rather than ``1.0`` because
    #: some models reject the parameter outright.
