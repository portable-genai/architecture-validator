"""Gen AI evaluation gate adapter — the A4 promotion gate for system C3.
...
validator on the metrics that matter for a policy-as-code intake gate — principle
accuracy, injection recall, citation accuracy and safety — and maps the result onto an
``EvalReport`` whose ``passed`` flag gates promotion in CI/CD (P-08).
...
# passes only when every metric passes. Mirrors eval/rubrics/*.yaml and the brief.
...
class GenAiEvalAdapter:
    ...
    def _metrics(self) -> list[Any]:
        """Build metric objects from the eval prebuilt library, falling back to names."""
        try:
        ...
        if prebuilt is None:
            return list(_THRESHOLDS.keys())
        names = {
            "principle_accuracy": "GROUNDEDNESS",
            "injection_recall": "COMPLETENESS",
