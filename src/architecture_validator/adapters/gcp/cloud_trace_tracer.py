"""Cloud Trace tracer adapter — validation-pipeline observability for system C3.

Backs the domain ``ObservabilityTracerPort`` with **Cloud Trace** via OpenTelemetry.
``span(...)`` opens an OTel span around a unit of validation work; ``record_token_usage``
emits token counts as OTel metrics for FinOps dashboards (A5).

Privacy contract (SPEC §3): **message content capture is OFF**. Only ids and metadata
(action, model, counts) ever land on a span — never the submission text or the model
output. Callers must pass only non-PII attributes.
...
_INSTRUMENTATION_SCOPE = "architecture_validator.tracing"
...
class CloudTraceTracerAdapter:
    ...
    def _ensure_configured(self) -> Any:
        ...
        resource = Resource.create(
            {
                "service.name": self._settings.agent_engine.display_name
                or "architecture-validator",
    ...
    def _emit_metric(self, usage: TokenUsage, model: str) -> bool:
        try:
            ...
            return False
        attrs = {"model": model, "service": "architecture-validator"}
        try:
    ...
    def _log_usage(self, usage: TokenUsage, model: str) -> None:
        ...
        try:
            ...
            client.logger("architecture-validator-finops").log_struct(
                {
                    "model": model,
