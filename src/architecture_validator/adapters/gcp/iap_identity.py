...
class IapIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # "/projects/<NUM>/apps/<ID>". Configure via ARCH_VALIDATOR_IAP_AUDIENCE; required in
        # secure mode.
        # Closed the SAME direction either way: unset and emptied both land on the empty
        # string, and ``resolve`` refuses to verify an assertion without an audience. The
        # collapse is deliberate; there is no safe default for an audience.
        self._audience = read_env_setting("ARCH_VALIDATOR_IAP_AUDIENCE").value
    ...
    def resolve(self, ctx: RequestContext) -> Principal:
        ...
        if not self._audience:
            raise IdentityError(
                "ARCH_VALIDATOR_IAP_AUDIENCE is not configured; cannot verify IAP assertion"
            )
