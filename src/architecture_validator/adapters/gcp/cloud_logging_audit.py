"""Cloud Logging WORM audit adapter — immutable audit trail for system C3.
...
``architecture-validator-audit`` log to a **locked log bucket** (WORM, ~7-year retention),
so records are write-once and immutable — the regulator-grade audit guarantee (A5, P-07).

C3 handles project metadata, not customer PII, so the event's summary fields carry the
submission summary and the verdict. This sink only serialises and writes; it does not
redact.
...
# Severity by decision: escalated/blocked verdicts surface for review at a higher log
# level than ordinary passed validations.
...
class CloudLoggingAuditAdapter:
    """Write ``AuditEvent`` records to the locked WORM log bucket."""
