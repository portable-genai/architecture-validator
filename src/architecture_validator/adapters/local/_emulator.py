...
There is no emulator for Agent Search / File Search, Gemini, OPA, Document AI or the Gen
AI eval service, so those adapters stay on the SDK-free workaround unconditionally. C3's
registry is the one store with an official emulator (Firestore), used here for the
in-process A2A AgentCard registry when opted in.
"""
