...
an in-process implementation rather than HTTP to a sibling service (a laptop runs one
app, not the whole platform). When the Firestore emulator is opted in
(``FIRESTORE_EMULATOR_HOST`` set AND the client lib imports), it also mirrors writes to
the emulator; the google client is imported lazily, only on that branch, so the default
local path imports no google-cloud package.
"""
