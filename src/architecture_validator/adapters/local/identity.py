...
These personas are an UNAUTHENTICATED grant of the arch-approver entitlement, so this
adapter refuses to construct unless a laptop profile was chosen deliberately: the profile
must actually be ``local`` or ``live`` (the same laptop posture with a local model
answering) AND (when the settings came from the environment)
``ARCH_VALIDATOR_PROFILE`` must have been set rather than inherited from the fallback. A
missing env var therefore fails closed instead of serving an intake gate with dev approvers.
The refusal is pure stdlib and happens at construction, before any credential is inspected,
so it does not depend on a cloud SDK being installed.
"""
...
# The persona id is the suffix of ``source`` after the colon. Entitlement group names are
# scoped to C3's architecture-validation domain (reviewers file/clear intake submissions;
# approvers sign off the maker-checker gate; auditors read the WORM trail). The auditor
# persona keeps ``group:audit`` so it matches the identity unit test.
_PERSONAS: tuple[Principal, ...] = (
    Principal(
        subject="demo.analyst@bank.example",
        principals=("group:arch-reviewer", "group:risk"),
        tenant="demo-bank",
    ...
    Principal(
        ...
        principals=("group:arch-reviewer", "group:risk", "group:arch-approver"),
        tenant="demo-bank",
        assurance="local-demo",
    ...
    Principal(
        ...
        principals=("group:arch-reviewer",),
        tenant="other-bank",
        assurance="local-demo",
...
class LocalPersonaIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "ARCH_VALIDATOR_PROFILE is not set, so the local profile was inherited rather "
                "than chosen; seeded dev personas grant the arch-approver entitlement with no "
                "authentication and are refused. Set ARCH_VALIDATOR_PROFILE=local deliberately "
                "for a dev or demo run, or ARCH_VALIDATOR_PROFILE=gcp for a real deployment."
            )
