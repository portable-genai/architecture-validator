...
In the full platform deployment, C3's A2A AgentCard is published to and resolved through
the shared ``agent-registry`` service rather than served only from C3's own
...
The base URL is read from ``AGENT_REGISTRY_URL`` with a localhost default (port 8083).
"""
...
class RemoteRegistryError(ValidatorError):
...
class RemoteRegistryAdapter:
    ...
    def _parse_card(body: dict) -> AgentCard:
        ...
        return AgentCard(
            ...
            skills=skills,
            provider=str(body.get("provider", "architecture-validator")),
        )
