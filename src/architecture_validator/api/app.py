...
# itself) and this emits it as `X-Answered-By` / `X-Search-Used` on the same response, with an
# `Access-Control-Expose-Headers` naming both: the console calls this service cross-origin when
# standalone, and a browser hides every unexposed header from script. The console's pills read
# those two headers, so what a pill names is what answered, never what configuration says would.
# A request that noted nothing (every route that calls no model) sends neither, and the pill
