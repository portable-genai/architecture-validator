...
against the two defects they exist for: an unconditional 0.0.0.0 bind, and a dev-origin
CORS fallback trusted in every profile.
"""
...
def _origins_for_profile(
    ...
    monkeypatch.delenv("ARCH_VALIDATOR_CORS_ORIGINS", raising=False)
...
def test_cors_fallback_needs_a_deliberate_local_profile(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Before the three-state fix an absent ARCH_VALIDATOR_PROFILE was read as a chosen
    ``local``, which handed the localhost dev origins cross-origin trust in any deployment
...
def test_explicit_allowlist_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ARCH_VALIDATOR_CORS_ORIGINS", "https://tenant.example")
...
def test_emptied_allowlist_grants_nothing_not_the_dev_origins(
    ...
    An operator who empties ARCH_VALIDATOR_CORS_ORIGINS named no origin, so nothing is
    trusted. The two-state read this replaced handed that case the unset default, which
    under a local profile is the localhost dev-origin relaxation.
    """
    monkeypatch.setenv("ARCH_VALIDATOR_CORS_ORIGINS", "")
    settings = dataclasses.replace(
...
def test_frame_ancestors_resolves_three_states(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_clickjacking_control_survives_an_emptied_allowlist(
    ...
    Red before the fix for ``raw=""``: the CSP directive was empty and the
    ``== "'self'"`` branch was skipped, so X-Frame-Options was absent as well and the
    clickjacking control silently disappeared from both headers at once.
    """
...
def test_local_profile_refuses_non_loopback_bind(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("ARCH_VALIDATOR_API_HOST", "0.0.0.0")
    monkeypatch.delenv("ARCH_VALIDATOR_ALLOW_INSECURE_DEMO", raising=False)
...
def test_a_wildcard_cors_origin_is_refused(monkeypatch: pytest.MonkeyPatch, raw: str) -> None:
    ...
    """
    monkeypatch.setenv("ARCH_VALIDATOR_CORS_ORIGINS", raw)
    with pytest.raises(ValueError, match="wildcard"):
...
def test_every_wildcard_spelling_is_refused_in_either_list(
    ...
    monkeypatch.setenv("ARCH_VALIDATOR_CORS_ORIGINS", entry)
    with pytest.raises(ValueError, match="wildcard"):
        app_module._cors_origins()
