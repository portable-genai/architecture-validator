...
TWO_STATE_READS_WITH_A_REASON: dict[str, str] = {
    "CHROME_PATH": (
        "scripts/arch_demo_playwright.py: which local Chrome binary Playwright drives. It "
        "selects a program on the presenter's own machine, names no host, origin or audience, "
        "and an unusable value fails the launch loudly."
    ),
    "DEMO_URL": (
        "scripts/arch_demo_playwright.py: which already-running demo server the browser "
        "walkthrough attaches to instead of spawning one. Presenter ergonomics on loopback; the "
        "demo server binds 127.0.0.1 and is not configurable."
    ),
    "SLOWMO_MS": (
        "scripts/arch_demo_playwright.py: how long Playwright pauses between steps so a human "
        "can follow the walkthrough. Pure presentation pacing; it reaches no service and grants "
        "nothing."
    ),
}
