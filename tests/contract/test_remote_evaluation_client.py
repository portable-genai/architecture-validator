...
whether C3 may be promoted, it delegates that decision to
...
from architecture_validator.adapters.platform.remote_evaluation import (
    RemoteEvaluationAdapter,
    RemoteEvaluationError,
)
from architecture_validator.config import Settings
from architecture_validator.domain.errors import ValidatorError
from architecture_validator.domain.models import EvalReport

_BASE = "https://hrz4.test"
_DATASET = "eval/datasets/golden_submissions.jsonl"
_DATASET_ID = "golden_submissions"
_DIGEST = "sha256:feedfacefeedfacefeedfacefeedfacefeedfacefeedfacefeedfacefeedface"
_BUNDLE = "rsk3-architecture-validator"

_PASSING_RESULTS = [
    {"metric": "principle_accuracy", "score": 0.94, "threshold": 0.90, "passed": True},
    {"metric": "injection_recall", "score": 0.97, "threshold": 0.95, "passed": True},
]
...
_FAILING_RESULTS = [
    {"metric": "principle_accuracy", "score": 0.71, "threshold": 0.90, "passed": False},
    {"metric": "injection_recall", "score": 0.97, "threshold": 0.95, "passed": True},
]
    ...
    body: dict[str, Any] = {
        ...
        "redteam_report": {
            ...
            "results": [
                ...
                {"case": "policy-bypass-01", "passed": True, "blocked": True},
            ],
        },
...
def test_evaluate_posts_a_structured_bundle_request_and_parses_results(
    ...
    assert [r.metric for r in report.results] == ["principle_accuracy", "injection_recall"]
    assert report.results[0].passed is False
    assert report.passed is False

    # The attested evidence SURVIVES the adapter. This is the assertion the old
    # ``_to_domain`` mapper failed: it rebuilt a locally-declared EvalReport from three
    # fields and silently dropped every durable identifier below, so a promotion arrived
    # with no way to retrieve the run that justified it. run_id would have read "".
...
def test_evaluate_REFUSES_a_verdict_that_contradicts_its_score(
    ...
    rows = [{"metric": "principle_accuracy", "score": 0.10, "threshold": 0.90, "passed": True}]
    respx.post(f"{_BASE}/v1/evaluations").mock(
...
def test_non_2xx_raises_a_domain_error(adapter: RemoteEvaluationAdapter) -> None:
    ...
    assert issubclass(RemoteEvaluationError, ValidatorError)
