# COMPLIANCE: `architecture-validator`
...
| Regulator | Reference family to validate | `architecture-validator` implementation seam |
| --- | --- | --- |
...
`architecture-validator` is the **meta-enforcer** of the toolkit: it operationalises every General Principle as a
...
- **`architecture-validator` enforces P-xx** (design-time): `architecture-validator` *checks* that a validated project honours the
...
| Principle | The rule `architecture-validator` uses to check a project | How `architecture-validator` itself honours it |
| --- | --- | --- |
...
| **P-03** Single in-country region | `rule_p_03`: FAIL if region not in the approved set and no signed-off exception | Region pinned `asia-southeast1` in config + Terraform; Org Policy `gcp.resourceLocations`. **One stated deviation in this repo's own stack:** **Agent Search serves no Cloud region at all** (`global`, `us` and `eu` only), so the retrieval corpus cannot be in-country at any setting: it defaults to `global`, which carries no residency guarantee. `us` or `eu` confines it to one jurisdiction and is the stronger choice where a residency obligation bites, and `gcp.resourceLocations` must be wide enough to permit whichever is chosen. The scanner grades the infrastructure it is pointed at, not its own settings, so this does not weaken the rule — but the repo that polices residency says where its own index sits rather than leaving it to be discovered |
| **P-04** Minimise data to the model (DLP) | `rule_p_04`: NEEDS_INFO if PII without DLP; NOT_APPLICABLE if no PII | **N/A for `architecture-validator` itself**: `architecture-validator` processes project metadata, not customer PII, so it runs no DLP redaction. (It still *checks* that the validated project honours P-04.) |
| **P-05** Grounding over fine-tuning | `rule_p_05`: FAIL if fine-tuning on PII; NEEDS_INFO if fine-tuning without grounding | **Enforced-by target.** `architecture-validator` never fine-tunes; the LLM only drafts prose grounded in findings + KB citations |
| **P-06** Human-in-the-loop / maker-checker | `rule_p_06`: FAIL if `maker_checker` not declared | `ReviewPolicy` sets `requires_human_review` on any FAIL or HIGH/CRITICAL finding (`domain/hitl.py`); the escalation is then ROUTED to the `human-review-console` maker-checker console (rule R8), not left as a per-repo boolean (`ports/review_router.py`, `adapters/*/review_router.py`) |
...
| **P-08** Eval-gated promotion | `rule_p_08`: FAIL if `eval_gate` not declared | `eval/run_eval.py` + `eval/rubrics/` (the only home for the bars) + `gate.yaml`, which runs `make check` on every pull request and blocks promotion unless it passes |
...
| **P-10** Resilience & graceful degradation | `rule_p_10`: NEEDS_INFO if no kill-switch / circuit-breaker | OPA outage falls back to the in-process evaluator; best-effort the cloud control-mapping toolkit/the data-residency validator degrade silently |
...
| --- | --- |
| **R1** (regulator-grounded controls) | Findings and injected requirements cite the principle and, where relevant, the reg KB via `KnowledgeBasePort` (`compliance-advisory` `/ask`) |
...
| **R5** (cross-system reuse / no duplication) | `architecture-validator` consumes `compliance-advisory` (reg KB), the cloud control-mapping toolkit (coverage) and the data-residency validator (residency) rather than re-deriving them; registers in `agent-registry`, audits to `agent-observability` |
| **R6** (any new project SHOULD pass `architecture-validator` at intake) | **`architecture-validator` IS the intake gate.** `POST /validate` and the `architecture-validator validate` CLI are the R6 enforcement point; the Cloud Build intake-gate wires it into CI/CD |
| **R8** (route `requires_human_review` to `human-review-console`) | Every shipped-policy `ValidationReport` is submitted to the `human-review-console` Human-Review & Maker-Checker Console via the shared `review-kit` client, mapping the verdict to a severity and dual-control flag: `local` enqueues to an in-memory outbox so the routing path runs offline, `gcp`/`platform` submit over S2S to `human-review-console`'s service intake (`HUMAN_REVIEW_URL`), `onprem` is the fail-fast sovereign placeholder. `gcp`/`platform` refuse to boot with no `HUMAN_REVIEW_URL`; a deployment can state `ARCH_VALIDATOR_REVIEW_ROUTING=off`, and every caller then reports `review_routing: "off"`; a failed hand-off is reported as `failed`, never swallowed. The payload carries project metadata only (no customer PII in this repo). `ports/review_router.py`, `adapters/{local,platform,onprem}/review_router.py`, `adapters/_review_payload.py`, `adapters/controls.py` |
...
repo, no `agent-guardrail-gateway` dependency. Its detector is deterministic, so a scan is reproducible
...
`requires_human_review` and is routed to the `human-review-console` maker-checker console (P-06 / R8); the
...
- **P-04 boundary PII redaction / `agent-guardrail-gateway`** is **N/A for this repo itself**: it handles
