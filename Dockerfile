# C3 Architecture & Requirements Validator — API service image.
#
# Builds the FastAPI service with the managed-stack extra ([gcp]) installed, so the
# deployed container talks to the OPA policy service / File Search / Gemini / Cloud Logging
# in asia-southeast1. The image is region-agnostic at build time; residency is enforced at
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    ARCH_VALIDATOR_SETTINGS=/app/config/settings.yaml \
    PORT=8088
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD python -c "import urllib.request,os; urllib.request.urlopen('http://127.0.0.1:'+os.environ.get('PORT','8088')+'/healthz')" || exit 1

# Use the shell form so $PORT is expanded at container start.
CMD exec uvicorn architecture_validator.api.app:app --host 0.0.0.0 --port ${PORT}
