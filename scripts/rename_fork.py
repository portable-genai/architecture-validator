...
Rewrites the package name, the CLI entry point, the ``ARCH_VALIDATOR_`` env-var prefix, and
...
changed), then run the gate. See docs/ADOPTING.md for the full checklist (principle set,
eval golden set, IdP wiring, region) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "architecture_validator"
_OLD_CLI = "architecture-validator"
_OLD_ENV_PREFIX = "ARCH_VALIDATOR_"
_OLD_RESOURCE = "architecture-validator"
_OLD_DIST = "architecture-validator"
...
def _replacements(args: argparse.Namespace) -> list[tuple[str, str]]:
    ...
    # Order matters: replace the longer, more specific strings first so a shorter stem
    # cannot consume part of a longer one. This repo's pypi distribution name equals the
    # CLI id (both "architecture-validator"), so the CLI rule also carries it; --dist defaults to
    # the --resource value.
    return [
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``ARCH_VALIDATOR_`` when it prefixes an UPPER
        # token (ARCH_VALIDATOR_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument("--resource", required=True, help="new resource id stem, e.g. acme-arch-gate")
    ...
    print("Then finish the human steps in docs/ADOPTING.md (principle set, eval, IdP, region).")
    return 0
