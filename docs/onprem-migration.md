...
The whole point of the ports-and-adapters shape is that Rsk3's exit story is
...
the agent wiring do not change. This is the same no-vendor-lock-in property Rsk3 enforces
...
To run Rsk3 on a sovereign / on-premise platform, implement these adapter bodies (the only
files that change):
...
| `ControlMappingClientPort` | `onprem/control_mapping.py` | The Rsk1 control-mapping service on-prem (best-effort coverage) |
...
| `EvaluationGatePort` | `onprem/evaluation.py` | An on-prem eval backend (the Hrz4 / P-08 promotion gate) |
...
| `ReviewRouterPort` | `onprem/review_router.py` | An on-prem route to the Hrz7 human-review / maker-checker console (rule R8) |
...
than a rewrite. That is the reversibility (P-02) and portability (P-12) Rsk3 checks for on
